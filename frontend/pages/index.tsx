...
export default memo(function Home() {
  ...
  const processData = useCallback(function processData(genome: string, file: File) {
    ...
    fetch("/api/data", requestParameters).then(response => response.json()).then(data => {
      if (data.ok) {
  ...
  return <main className="max-w-6xl mx-auto px-16">
      {/* Main header. */}
      ...
      <section className="mb-10">
        ...
        {listIsOpen && <div onClick={() => setListIsOpen(false)} className="block fixed inset-0 z-40"></div>}
        {/* List with all proteins. */}
        <div className="relative z-50 mb-4">
          <button onClick={listOpenHandler} disabled={!proteinsData} className="text-left max-w-sm text-lg font-bold flex justify-center items-center gap-2 disabled:pointer-events-none disabled:text-gray-400">
            <span className="truncate">
              {!proteinsData ? "Wprowadź sekwencję DNA" : proteinsData.proteins.map((protein: IProtein, index) => {
              if (protein === currentProtein) {
                return index + 1 + ". " + currentProtein.protein;
              }
            })}
            </span>{" "}
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className={`block w-12 h-5 transform transition-transform ${!listIsOpen ? "rotate-0" : "rotate-90"}`}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
            </svg>
          </button>
          <ul className={`absolute top-full mt-3 flex flex-col justify-start bg-gray-50 max-w-2xl max-h-96 overflow-y-scroll rounded-lg transform transition-all duration-300 ${!listIsOpen ? "-translate-y-10 opacity-0 pointer-events-none" : "translate-y-0 opacity-100 pointer-events-auto shadow-md"}`}>
            {/* List of the proteins in each protein. */}
            {proteinsData && proteinsData.proteins.map((protein: IProtein, index) => {
            return <li key={index}>
                    <button className={`grid grid-cols-[auto_1fr] gap-2 px-5 py-3 w-full break-all text-left hover:bg-gray-300 ${protein.protein === currentProtein?.protein ? "bg-gray-200" : ""}`} onClick={() => changeProteinHandler(protein)}>
                      <span>{`${index + 1}.`}</span>
